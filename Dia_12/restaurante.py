...
def recibo():
    ...
    for comida in texto_comida:
        if comida.get() != '0':
            texto_recibo.insert(END, f'{lista_comidas[c]}\t\t{comida.get()}\t'
                                f'${int(comida.get()) * precios_comida[c]}\n')
        c +=1

    b = 0
    for bebida in texto_bebida:
        if bebida.get() != '0':
            texto_recibo.insert(END, f'{lista_bebidas[b]}\t\t{bebida.get()}\t'
                                f'${int(bebida.get()) * precios_bebida[b]}\n')
        b +=1

    p = 0
    for postre in texto_postre:
        if postre.get() != '0':
            texto_recibo.insert(END, f'{lista_postres[p]}\t\t{postre.get()}\t'
                                f'${int(postre.get()) * precios_postres[p]}\n')
        p +=1
    texto_recibo.insert(END, f'.' *91 + '\n')
...
# Evitrar que la pantalla se cierre

# Tamaño de la ventana
aplicacion.geometry('1145x630+0+0')

# Evitar maximizar
aplicacion.resizable(False,
                     False)
...
# Botones
botones = ['total', 'recibido', 'guardar', 'resetear']
botones_creados = []
columnas = 0
